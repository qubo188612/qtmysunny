...
void qtmysunnyDlg::img_windowshow(bool b_show,QLabel *lab_show)
{
    if(b_show==true)
    {
        ...
        //读取机器人信息
        int real_readnum=modbus_read_registers(m_mcs->resultdata.ctx_robotset,0x00,2,m_mcs->resultdata.red_robotset);
        if(real_readnum<0)
        {
            if(ui->checkBox->isChecked()==false)
...
void qtmysunnyDlg::init_show_pos_list()
{
    ...
    float Y=(int16_t)pos_data[1]/0.01;
    float Z=(int16_t)pos_data[2]/0.01;
    ui->label_10->setText(QString::number(Y));
    ui->label_11->setText(QString::number(Z));
}
